...
# Real-time logging: force every print to flush so progress is visible during long crawls
...
class CareerPortalScanner:
    def __init__(self, input_csv="company_Career_seed.csv", output_csv="scraped_jobs_v7.csv",
                 ...
                 allow_synthetic=False, skip_preflight=False):
        ...
        for url_i,url in enumerate(urls,1):
            ...
            for sent in self.detector.split_sentences(desc):
                if re.search(r"\b(?:eu\s+)?blue[- ]?card|blaue karte|carta blu|blauwe kaart|carte bleue|tarjeta azul\b",sent,re.I):
                    blue_ev=sent[:300]
                    if self.detector.NEGATION.search(sent): blue="N"
                    elif self.detector.POSITIVE_VERBS.search(sent): blue="Y"
                    break
    ...
    def _enrich_support_from_detail(self, rows, max_fetch=None, use_browser=True):
        ...
        try:
            for r in targets:
                ...
                elif sup["visa"]["verdict"] == VERDICT_NO:
                    r["EU Blue Card"] = "N"
    ...
    def execute_crawler(self):
        ...
        with cf.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = [executor.submit(crawl_target, i, row) for i, row in enumerate(targets, 1)]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Career Portal Scanner v7\n"
            "- fresh output by default (never silently appends)\n"
            "- recruiter rows written to a separate file\n"
            "- provider API first; DOM fallback second\n"
            "- Unknown used when evidence is missing"
        ),
        formatter_class=argparse.RawTextHelpFormatter,
    )
    parser.add_argument("--input", default="company_Career_seed.csv",
                        help="v7 seed CSV path")
    parser.add_argument("--output", default=None,
                        help="Direct-employer output path. Default: scraped_jobs_v7.csv")
    parser.add_argument("--workers", type=int, default=3)
    parser.add_argument("--detail", action="store_true",
                        help="Visit detail pages for explicit support/type/location evidence")
    parser.add_argument("--resume", action="store_true",
                        help="Append only unseen canonical requisitions to an existing exact-v7 schema")
    parser.add_argument("--allow-synthetic", action="store_true",
                        help="Allow invented #job= URLs (not recommended; default quarantines them)")
    parser.add_argument("--skip-preflight", action="store_true",
                        help="Skip the connectivity pre-flight gate (not recommended)")
    args = parser.parse_args()

    output = args.output or "scraped_jobs_v7.csv"
