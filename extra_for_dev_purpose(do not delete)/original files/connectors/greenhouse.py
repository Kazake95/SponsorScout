...
class GreenhouseConnector(BaseConnector):
    ...
    def _extract_token(self, url: str) -> str:
        ...
        # Custom domain: only attempt hostname-derived slug when the URL
        # explicitly points at a greenhouse subdomain or embed path.
        # Previously this guessed a slug for ANY careers.company.com URL,
        # causing non-Greenhouse companies to hit the wrong API endpoint.
        from urllib.parse import urlparse
        try:
            parsed = urlparse(url)
            hostname = parsed.hostname or ""
            path = parsed.path or ""
            # Only guess from hostname when there is a clear Greenhouse signal
            # in the URL (e.g. /greenhouse/ embed path, or greenhouse in the hostname)
            if "greenhouse" in hostname or "/greenhouse" in path.lower():
