...
class WorkdayConnector(BaseConnector):
    ...
    def fetch_jobs(self, company):
        ...
        with http_session() as session:
            ...
            exact_subdomain = self._extract_workday_subdomain(careers_url)

            if tenant and site:
                subdomains_to_try = []
                if exact_subdomain and exact_subdomain != tenant:
                    subdomains_to_try.append(exact_subdomain)
                subdomains_to_try.append(tenant)
    ...
    def _extract_workday_subdomain(self, url: str) -> str:
        """Extract the exact subdomain including wdN prefix if present."""
        m = re.match(r"https?://([a-zA-Z0-9_-]+(?:\.wd\d+)?)\.myworkdayjobs\.com", url)
