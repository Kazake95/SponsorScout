...
class SponsorScoutApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # Create tab frames
        self.dashboard_tab = tk.Frame(self.tabs, bg="#f0f2f5")
        self.search_tab = tk.Frame(self.tabs, bg="#f0f2f5")
        self.tailor_tab = tk.Frame(self.tabs, bg="#f0f2f5")
        self.ai_assistant_tab = tk.Frame(self.tabs, bg="#f0f2f5")
        self.applications_tab = tk.Frame(self.tabs, bg="#f0f2f5")
        self.health_tab = tk.Frame(self.tabs, bg="#f0f2f5")
        self.tools_tab = tk.Frame(self.tabs, bg="#f0f2f5")
        self.ai_settings_tab = tk.Frame(self.tabs, bg="#f0f2f5")
    ...
    def _build_search_tab(self):
        ...
        self._country_cmb = cmb(r1, self.country_var, ["All"], 18)
        self._country_cmb.pack(side="left", padx=(3, 10))
        self._refresh_country_dropdown()
        ...
        cols = ("Title", "Company", "Industry", "Location",
                "Remote", "Sponsor", "BluCard", "Reloc", "AI ★", "URL")
        widths = {
            "Title": 220, "Company": 130, "Industry": 100,
    ...
    def _copy_rating_prompt(self):
        ...
        # Fetch description & country reliably from DB
        try:
            conn = get_connection(DB_PATH)
            row = conn.execute("SELECT description, country FROM jobs WHERE url=?", (url,)).fetchone()
            description = row["description"] if row else ""
            country = row["country"] if row else ""
            conn.close()
        except Exception as exc:
            logger.exception("Failed to load job description and country for AI rating")
            description = ""
            country = ""

        # Shifted indices (0: Title, 1: Company, 2: Location, 3: Remote, 4: Sponsor, 5: BluCard, 6: Reloc, 7: AI ★, 8: URL)
        spons   = int(v[4]) if str(v[4]).lstrip("-").isdigit() else 0
        remote  = str(v[3])
        bluecard = v[5] == "✓"
        reloc    = v[6] == "✓"
    ...
    def _update_ai_rating_in_tree(self, rating: int, sel: tuple):
        ...
        try:
            ...
            values[7] = rating_str  # AI ★ column index shifted from 8 to 7
    ...
    def _build_dashboard_tab(self):
        ...
        for c in co_cols:
            ...
            self.co_tree.column(c, width=150, anchor="center")
    ...
    def _build_health_tab(self):
        ...
        for c in h_cols:
            ...
            self.h_tree.column(c, width=h_widths.get(c, 120), anchor="center")
    ...
    def _build_tools_tab(self):
        ...
        ttk.Button(dq, text="Clear stale data",
                   command=self._clear_stale_data).pack(anchor="w", pady=(4, 0))
    ...
    def _refresh_country_dropdown(self):
        """Populate the Country filter with distinct job location countries from the DB.

        Falls back to the hardcoded EU list if the DB is empty or unreachable.
        Always called after each scan batch completes so the dropdown stays current.
        """
        try:
            countries = get_distinct_job_countries(DB_PATH)
        except Exception:
            countries = []
        ...
        current = self.country_var.get()
        values = ["All"] + sorted(countries)
        if hasattr(self, "_country_cmb"):
            self._country_cmb["values"] = values
    ...
    def load_results(self):
        ...
        if hasattr(self, "tree"):
            ...
            for r in rows:
                ...
                # Removed r["country"] from search table values insertion
                self.tree.insert("", "end", values=(
                    r["title"], r["company"],
                    r["industry"] if "industry" in r.keys() else "",
    ...
    def load_health(self):
        ...
        try:
            ...
            if hasattr(self, "_health_note_var"):
    ...
    def _clear_stale_data(self):
        try:
            ...
            messagebox.showinfo(
                "Stale data cleared",
                f"Removed {deleted} expired job(s) from the database.")
            self.load_dashboard()
            self.load_results()
        except Exception as exc:
            messagebox.showerror("Error", str(exc))
