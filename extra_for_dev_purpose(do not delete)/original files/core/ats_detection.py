...
def detect_ats_from_links(links: list[str]) -> Tuple[str, str]:
    ...
    for url in links:
        ...
        # SAP SuccessFactors
        m = re.search(r"([a-zA-Z0-9_-]+)\.successfactors\.(com|eu)", url)
        if m:
            return "successfactors", m.group(1)

        # Oracle Taleo
        m = re.search(r"([a-zA-Z0-9_-]+)\.taleo\.net", url)
        if m:
            return "taleo", m.group(1)

        # Oracle HCM Cloud
        m = re.search(r"([a-zA-Z0-9_-]+)\.oraclecloudhcm\.com", url)
        if m:
            return "oraclehcm", m.group(1)

        # Eightfold AI
        m = re.search(r"([a-zA-Z0-9_-]+)\.eightfold\.ai", url)
        if m:
            return "eightfold", m.group(1)

        # Phenom People
        m = re.search(r"([a-zA-Z0-9_-]+)\.phenom(?:people)?\.com", url)
        if m:
            return "phenom", m.group(1)

        # Wise Jobs (Workable-powered)
        m = re.search(r"wise\.jobs", url)
        if m:
            return "workable", "wise"

        # Babbel Jobs (Workable-powered)
        m = re.search(r"jobs\.babbel\.com", url)
        if m:
            return "workable", "babbel"

        # Booking.com Jobs (Workable-powered)
        m = re.search(r"jobs\.booking\.com/([^/?#]+)", url)
        if m:
            return "workable", m.group(1)
